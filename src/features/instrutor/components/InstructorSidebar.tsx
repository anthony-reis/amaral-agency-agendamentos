...
export function InstructorSidebar({ escola, name, category, logoUrl, onLogout }: Props) {
  ...
  const NavContent = () => (
    <>
      ...
      <div className="flex items-center gap-3 px-5 py-4 border-b border-[--p-border]">
        {logoUrl ? (
          <img src={logoUrl} alt={name} className="w-8 h-8 rounded-lg object-cover shrink-0" />
        ) : (
    ...
    <>
      ...
      <div className="lg:hidden w-full flex items-center justify-between px-4 py-3 bg-[--p-bg-card] border-b border-[--p-border] sticky top-0 z-30">
        <div className="flex items-center gap-2.5">
          {logoUrl ? (
            <img src={logoUrl} alt={name} className="w-6 h-6 rounded object-cover" />
          ) : (
