...
export async function reconciliarPedido(
  ...
  if (!accessToken) return { status: pedido.status, creditos_liberados: pedido.creditos_liberados }

  try {
    const payment = await buscarPaymentPorExternalReference(pedido.id, accessToken)
