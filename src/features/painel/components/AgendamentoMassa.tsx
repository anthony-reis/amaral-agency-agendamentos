...
export function AgendamentoMassa({
  alunos,
  instrutores,
  autoescola_id,
}: Props) {
  ...
  const [alunoSelecionado, setAlunoSelecionado] =
    useState<AlunoComCreditos | null>(null);
  ...
  function toggleSlot(
    ...
    if (
      jaTemDia?.instructorName === instructorName &&
      jaTemDia?.timeSlot === timeSlot
    ) {
    ...
    if (jaTemDia) {
      ...
      setSelecionados((prev) =>
        prev.map((s) =>
          s.date === key ? { ...s, instructorName, timeSlot } : s,
        ),
      );
    } else {
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-[--p-text-3]">
            Agende múltiplas aulas de uma só vez
          </p>
        </div>
      </div>
            ...
            return (
              <div key={n} className="flex items-center gap-2">
                ...
                {idx < stepLabels.length - 1 && (
                  <div
                    className={`h-px w-4 sm:w-8 ${done ? "bg-[#0ea5e9]/40" : "bg-[--p-border]"}`}
                  />
                )}
              </div>
            );
        ...
        {step === 1 && (
          ...
          >
            <div className="px-5 py-4 border-b border-[--p-border]">
              <h2 className="font-semibold text-[--p-text-1] text-sm">
                Selecionar aluno
              </h2>
            </div>
            ...
            <ul className="divide-y divide-[--p-border] max-h-80 overflow-y-auto">
              {alunosFiltrados.length === 0 ? (
                <li className="px-5 py-8 text-center text-sm text-[--p-text-3]">
            ...
            <div className="bg-[--p-bg-card] rounded-2xl border border-[--p-border] overflow-hidden">
              ...
              <div className="px-5 py-5 space-y-5">
                ...
                <div>
                  ...
                  <div className="flex flex-col lg:flew-row gap-3">
                    {(["CARRO", "MOTO"] as const).map((cat) => {
                      const credits =
                        cat === "CARRO" ? creditosCarro : creditosMoto;
                  ...
                  {creditosCategoria === 0 && (
                    <p className="mt-2 text-xs text-red-400">
                      Aluno sem créditos para esta categoria.
                    </p>
                  )}
                </div>
                ...
                <div>
                  <label className="block text-xs font-semibold text-[--p-text-2] mb-2 uppercase tracking-wide">
                    ...
                    <span className="normal-case font-normal text-[--p-text-3]">
                      (opcional)
                    </span>
                  </label>
                  <select
                  ...
                  >
                    ...
                    {instrutores
                      .filter(
                        (i) =>
                          i.category === category || i.category === "AMBOS",
                      )
                      .map((i) => (
          ...
          >
            ...
            <div className="bg-[--p-bg-card] rounded-2xl border border-[--p-border] px-5 py-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-[--p-text-3]">
                  {category} · {slotsCount} de {creditosCategoria} crédito
                  {creditosCategoria !== 1 ? "s" : ""} selecionado
                  {slotsCount !== 1 ? "s" : ""}
                </p>
              </div>
              <div className="flex items-center gap-3">
            ...
            {dias.length === 0 ? (
              ...
              <div className="space-y-2">
                {dias.map((dia) => {
                  ...
                  const slotDoDia = selecionados.find(
                    (s) => s.date === dia.date,
                  );
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        <span className="flex-1 text-sm font-medium text-[--p-text-1]">
                          {dia.label}
                        </span>
                        ...
                        {slotDoDia ? (
                          ...
                          <span className="text-xs text-[--p-text-3]">
                            {dia.instrutores.reduce(
                              (acc, i) => acc + i.horarios.length,
                              0,
                            )}{" "}
                            horários
                          </span>
                              ...
                              {dia.instrutores.map((inst) => (
                                <div key={inst.nome}>
                                  <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <span className="text-xs font-semibold text-[--p-text-2] uppercase tracking-wide">
                                      {inst.nome}
                                    </span>
                                  </div>
                                  <div className="flex flex-wrap gap-2">
                                    {inst.horarios.map((h) => {
                                      const isSelected =
                                        slotDoDia?.instructorName ===
                                          inst.nome &&
                                        slotDoDia?.timeSlot === h;
                                      const maxReached =
                                        slotsCount >= creditosCategoria &&
                                        !slotDoDia;
            ...
            {slotsCount > 0 && (
              <div className="flex justify-end">
                ...
                >
                  Ver resumo ({slotsCount} aula{slotsCount !== 1 ? "s" : ""}){" "}
                ...
                {resultado.success ? (
                  <>
                    ...
                    <h3 className="text-base font-bold text-[--p-text-1] mb-1">
                      {resultado.created} aula
                      {resultado.created !== 1 ? "s" : ""} agendada
                      {resultado.created !== 1 ? "s" : ""}!
                    </h3>
              ...
              <div className="bg-[--p-bg-card] rounded-2xl border border-[--p-border] overflow-hidden">
                <div className="px-5 py-4 border-b border-[--p-border]">
                  ...
                  <p className="text-xs text-[--p-text-3] mt-0.5">
                    {alunoSelecionado.name} · {category} · {slotsCount} aula
                    {slotsCount !== 1 ? "s" : ""}
                  </p>
                </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[--p-border]">
                      <th className="text-left px-5 py-2.5 text-xs font-semibold text-[--p-text-3] uppercase tracking-wide">
                        Data
                      </th>
                      <th className="text-left px-5 py-2.5 text-xs font-semibold text-[--p-text-3] uppercase tracking-wide">
                        Instrutor
                  ...
                  <tbody className="divide-y divide-[--p-border]">
                    {selecionados.map((s) => (
                      <tr key={s.date} className="hover:bg-[--p-hover]">
                        <td className="px-5 py-3 text-[--p-text-1]">
                          {s.label}
                        </td>
                        <td className="px-5 py-3 text-[--p-text-2]">
                          {s.instructorName}
                        </td>
                        <td className="px-5 py-3 text-[--p-text-2]">
                          {s.timeSlot}
                ...
                <div className="px-5 py-4 border-t border-[--p-border] bg-[--p-bg-input]/50 flex items-center justify-between">
                  <div className="text-sm text-[--p-text-3]">
                    <span className="font-semibold text-[--p-text-1]">
                      {slotsCount}
                    </span>{" "}
                    crédito{slotsCount !== 1 ? "s" : ""} serão consumidos
                  </div>
                  <div className="flex items-center gap-3">
                    <button
                      onClick={() => setStep(3)}
